...
function getWebGLContext(e) {
	var r, t, n = {
		...
		a = !! i;
	a || (i = e.getContext("webgl", n) || e.getContext("experimental-webgl", n)), t = a ? (i.getExtension("EXT_color_buffer_float"), i.getExtension("OES_texture_float_linear")) : (r = i.getExtension("OES_texture_half_float"), i.getExtension("OES_texture_half_float_linear")), i.clearColor(0, 0, 0, 1);
	var o, l, u, c = a ? i.HALF_FLOAT : r.HALF_FLOAT_OES;
	return u = a ? (o = getSupportedFormat(i, i.RGBA16F, i.RGBA, c), l = getSupportedFormat(i, i.RG16F, i.RG, c), getSupportedFormat(i, i.R16F, i.RED, c)) : (o = getSupportedFormat(i, i.RGBA, i.RGBA, c), l = getSupportedFormat(i, i.RGBA, i.RGBA, c), getSupportedFormat(i, i.RGBA, i.RGBA, c)), {
		gl: i,
		ext: {
			formatRGBA: o,
			formatRG: l,
...
function getSupportedFormat(e, r, t, n) {
	if (!supportRenderTextureFormat(e, r, t, n)) switch (r) {
	case e.R16F:
...
var Material = function() {
		function t(e, r) {
			_classCallCheck(this, t), this.vertexShader = e, this.fragmentShaderSource = r, this.programs = [], this.activeProgram = null, this.uniforms = []
		}
		return _createClass(t, [{
			...
			value: function(e) {
				...
				if (null == n) {
					var i = compileShader(gl.FRAGMENT_SHADER, this.fragmentShaderSource, e);
					n = createProgram(this.vertexShader, i), this.programs[r] = n
				}
				n != this.activeProgram && (this.uniforms = getUniforms(n), this.activeProgram = n)
			}
		}, {
			key: "bind",
			value: function() {
	...
	Program = function() {
		function t(e, r) {
			_classCallCheck(this, t), this.uniforms = {}, this.program = createProgram(e, r), this.uniforms = getUniforms(this.program)
		}
		return _createClass(t, [{
			key: "bind",
			value: function() {
...
function createProgram(e, r) {
	...
	if (gl.attachShader(t, e), gl.attachShader(t, r), gl.linkProgram(t), !gl.getProgramParameter(t, gl.LINK_STATUS)) throw gl.getProgramInfoLog(t);
	return t
}
...
var dye, velocity, divergence, curl, pressure, bloom, sunrays, sunraysTemp, baseVertexShader = compileShader(gl.VERTEX_SHADER, "\n    precision highp float;\n\n    attribute vec2 aPosition;\n    varying vec2 vUv;\n    varying vec2 vL;\n    varying vec2 vR;\n    varying vec2 vT;\n    varying vec2 vB;\n    uniform vec2 texelSize;\n\n    void main () {\n        vUv = aPosition * 0.5 + 0.5;\n        vL = vUv - vec2(texelSize.x, 0.0);\n        vR = vUv + vec2(texelSize.x, 0.0);\n        vT = vUv + vec2(0.0, texelSize.y);\n        vB = vUv - vec2(0.0, texelSize.y);\n        gl_Position = vec4(aPosition, 0.0, 1.0);\n    }\n"),
	...
	bloomFramebuffers = [],
	ditheringTexture = createTextureAsync("assets/img/background.png"),
...
function initFramebuffers() {
	...
	dye = null == dye ? createDoubleFBO(r.width, r.height, n.internalFormat, n.format, t, o) : resizeDoubleFBO(dye, r.width, r.height, n.internalFormat, n.format, t, o), velocity = null == velocity ? createDoubleFBO(e.width, e.height, i.internalFormat, i.format, t, o) : resizeDoubleFBO(velocity, e.width, e.height, i.internalFormat, i.format, t, o), divergence = createFBO(e.width, e.height, a.internalFormat, a.format, t, gl.NEAREST), curl = createFBO(e.width, e.height, a.internalFormat, a.format, t, gl.NEAREST), pressure = createDoubleFBO(e.width, e.height, a.internalFormat, a.format, t, gl.NEAREST), initBloomFramebuffers(), initSunraysFramebuffers()
}
...
function initSunraysFramebuffers() {
	var e = getResolution(config.SUNRAYS_RESOLUTION),
		...
		t = ext.formatR,
		n = ext.supportLinearFiltering ? gl.LINEAR : gl.NEAREST;
	sunrays = createFBO(e.width, e.height, t.internalFormat, t.format, r, n), sunraysTemp = createFBO(e.width, e.height, t.internalFormat, t.format, r, n)
}
function createFBO(e, r, t, n, i, a) {
	gl.activeTexture(gl.TEXTURE0);
	var o = gl.createTexture();
	gl.bindTexture(gl.TEXTURE_2D, o), gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, a), gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, a), gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE), gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE), gl.texImage2D(gl.TEXTURE_2D, 0, t, e, r, 0, n, i, null);
	var l = gl.createFramebuffer();
	return gl.bindFramebuffer(gl.FRAMEBUFFER, l), gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, o, 0), gl.viewport(0, 0, e, r), gl.clear(gl.COLOR_BUFFER_BIT), {
		texture: o,
		fbo: l,
		width: e,
		height: r,
...
function resizeFBO(e, r, t, n, i, a, o) {
	...
	return copyProgram.bind(), gl.uniform1i(copyProgram.uniforms.uTexture, e.attach(0)), blit(l.fbo), l
}
function resizeDoubleFBO(e, r, t, n, i, a, o) {
	return e.width == r && e.height == t || (e.read = resizeFBO(e.read, r, t, n, i, a, o), e.write = createFBO(r, t, n, i, a, o), e.width = r, e.height = t, e.texelSizeX = 1 / r, e.texelSizeY = 1 / t), e
}
function createTextureAsync(e) {
	...
	var t = {
		...
		height: 1,
		attach: function(e) {
			return gl.activeTexture(gl.TEXTURE0 + e), gl.bindTexture(gl.TEXTURE_2D, r), e
		}
	},
		n = new Image;
	return n.onload = function() {
		t.width = n.width, t.height = n.height, gl.bindTexture(gl.TEXTURE_2D, r), gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, n)
	}, n.src = e, t
}
...
var lastUpdateTime = Date.now(),
	...
	changeColor = function() {
		document.querySelector(".content-inner").style.background = "unset", document.querySelector(".shape").style.fill = "#1e1f21"
	},
	initBackground = function e() {
		e.loaded || (e.loaded = !0, changeColor(), updateKeywords(), initFramebuffers(), multipleSplats(parseInt(20 * Math.random()) + 5), update(!0))
	};
window.addEventListener(visibilityChangeEvent, initBackground), window.addEventListener("DOMContentLoaded", initBackground);
var animationID = null;

function update(e) {
	var r = calcDeltaTime();
	resizeCanvas() && initFramebuffers(), updateColors(r), applyInputs(), config.PAUSED || step(r), render(null), animationID = requestAnimationFrame(update)
}
...
function step(e) {
	...
	for (var r = 0; r < config.PRESSURE_ITERATIONS; r++) gl.uniform1i(pressureProgram.uniforms.uPressure, pressure.read.attach(1)), blit(pressure.write.fbo), pressure.swap();
	...
	gl.uniform1i(advectionProgram.uniforms.uVelocity, t), gl.uniform1i(advectionProgram.uniforms.uSource, t), gl.uniform1f(advectionProgram.uniforms.dt, e), gl.uniform1f(advectionProgram.uniforms.dissipation, config.VELOCITY_DISSIPATION), blit(velocity.write.fbo), velocity.swap(), gl.viewport(0, 0, dye.width, dye.height), ext.supportLinearFiltering || gl.uniform2f(advectionProgram.uniforms.dyeTexelSize, dye.texelSizeX, dye.texelSizeY), gl.uniform1i(advectionProgram.uniforms.uVelocity, velocity.read.attach(0)), gl.uniform1i(advectionProgram.uniforms.uSource, dye.read.attach(1)), gl.uniform1f(advectionProgram.uniforms.dissipation, config.DENSITY_DISSIPATION), blit(dye.write.fbo), dye.swap()
}
function render(e) {
	...
	config.TRANSPARENT || drawColor(n, normalizeColor(config.BACK_COLOR)), null == e && config.TRANSPARENT && drawCheckerboard(n), drawDisplay(n, r, t)
}
function drawColor(e, r) {
	colorProgram.bind(), gl.uniform4f(colorProgram.uniforms.color, r.r, r.g, r.b, 1), blit(e)
}
...
function drawDisplay(e, r, t) {
	if (displayMaterial.bind(), config.SHADING && gl.uniform2f(displayMaterial.uniforms.texelSize, 1 / r, 1 / t), gl.uniform1i(displayMaterial.uniforms.uTexture, dye.read.attach(0)), config.BLOOM) {
...
function applyBloom(e, r) {
	if (!(bloomFramebuffers.length < 2)) {
		...
		var n = config.BLOOM_THRESHOLD * config.BLOOM_SOFT_KNEE + 1e-4,
			...
			a = 2 * n,
			o = .25 / n;
		gl.uniform3f(bloomPrefilterProgram.uniforms.curve, i, a, o), gl.uniform1f(bloomPrefilterProgram.uniforms.threshold, config.BLOOM_THRESHOLD), gl.uniform1i(bloomPrefilterProgram.uniforms.uTexture, e.attach(0)), gl.viewport(0, 0, t.width, t.height), blit(t.fbo), bloomBlurProgram.bind();
		for (var l = 0; l < bloomFramebuffers.length; l++) {
			var u = bloomFramebuffers[l];
			gl.uniform2f(bloomBlurProgram.uniforms.texelSize, t.texelSizeX, t.texelSizeY), gl.uniform1i(bloomBlurProgram.uniforms.uTexture, t.attach(0)), gl.viewport(0, 0, u.width, u.height), blit(u.fbo), t = u
		}
		gl.blendFunc(gl.ONE, gl.ONE), gl.enable(gl.BLEND);
		for (var c = bloomFramebuffers.length - 2; 0 <= c; c--) {
			var g = bloomFramebuffers[c];
			gl.uniform2f(bloomBlurProgram.uniforms.texelSize, t.texelSizeX, t.texelSizeY), gl.uniform1i(bloomBlurProgram.uniforms.uTexture, t.attach(0)), gl.viewport(0, 0, g.width, g.height), blit(g.fbo), t = g
		}
		gl.disable(gl.BLEND), bloomFinalProgram.bind(), gl.uniform2f(bloomFinalProgram.uniforms.texelSize, t.texelSizeX, t.texelSizeY), gl.uniform1i(bloomFinalProgram.uniforms.uTexture, t.attach(0)), gl.uniform1f(bloomFinalProgram.uniforms.intensity, config.BLOOM_INTENSITY), gl.viewport(0, 0, r.width, r.height), blit(r.fbo)
	}
}
function applySunrays(e, r, t) {
	gl.disable(gl.BLEND), sunraysMaskProgram.bind(), gl.uniform1i(sunraysMaskProgram.uniforms.uTexture, e.attach(0)), gl.viewport(0, 0, r.width, r.height), blit(r.fbo), sunraysProgram.bind(), gl.uniform1f(sunraysProgram.uniforms.weight, config.SUNRAYS_WEIGHT), gl.uniform1i(sunraysProgram.uniforms.uTexture, r.attach(0)), gl.viewport(0, 0, t.width, t.height), blit(t.fbo)
}
function blur(e, r, t) {
	blurProgram.bind();
	for (var n = 0; n < t; n++) gl.uniform2f(blurProgram.uniforms.texelSize, e.texelSizeX, 0), gl.uniform1i(blurProgram.uniforms.uTexture, e.attach(0)), blit(r.fbo), gl.uniform2f(blurProgram.uniforms.texelSize, 0, e.texelSizeY), gl.uniform1i(blurProgram.uniforms.uTexture, r.attach(0)), blit(e.fbo)
}
...
function updatePointerDownData(e, r, t, n) {
	e.id = r, e.down = !0, e.moved = !1, e.texcoordX = t / canvas.width, e.texcoordY = 1 - n / canvas.height, e.prevTexcoordX = e.texcoordX, e.prevTexcoordY = e.texcoordY, e.deltaX = 0, e.deltaY = 0, e.color = generateColor()
}
function updatePointerMoveData(e, r, t) {
	e.prevTexcoordX = e.texcoordX, e.prevTexcoordY = e.texcoordY, e.texcoordX = r / canvas.width, e.texcoordY = 1 - t / canvas.height, e.deltaX = correctDeltaX(e.texcoordX - e.prevTexcoordX), e.deltaY = correctDeltaY(e.texcoordY - e.prevTexcoordY), e.moved = 0 < Math.abs(e.deltaX) || 0 < Math.abs(e.deltaY)
}
...
function HSVtoRGB(e, r, t) {
	var n, i, a, o, l, u, c, g;
	switch (u = t * (1 - r), c = t * (1 - (l = 6 * e - (o = Math.floor(6 * e))) * r), g = t * (1 - (1 - l) * r), o % 6) {
	case 0:
		n = t, i = g, a = u;
		break;
	case 1:
...
function hashCode(e) {
	...
	for (var r = 0, t = 0; t < e.length; t++) r = (r << 5) - r + e.charCodeAt(t), r |= 0;
	return r
}
window.addEventListener("touchend", function(e) {
	for (var t = e.changedTouches, r = function(r) {
	...
	if (!switchPage || !switchPage.switched) {
		...
		null == n && (n = new pointerPrototype), updatePointerDownData(n, -1, r, t)
	}
}), document.addEventListener("mousemove", function(e) {
	if (!switchPage || !switchPage.switched) {
		var r = pointers[0];
		if (r.down) updatePointerMoveData(r, scaleByPixelRatio(e.pageX), scaleByPixelRatio(e.pageY))
	}
	...
	if (!switchPage || !switchPage.switched) {
		e.preventDefault();
		for (var r = e.targetTouches; r.length >= pointers.length;) pointers.push(new pointerPrototype);
		for (var t = 0; t < r.length; t++) {
	...
	if (!switchPage || !switchPage.switched) {
		...
		for (var r = e.targetTouches, t = 0; t < r.length; t++) {
			var n = pointers[t + 1];
			if (n.down) updatePointerMoveData(n, scaleByPixelRatio(r[t].pageX), scaleByPixelRatio(r[t].pageY))
		}
	}
}, !1), document.addEventListener("touchend", function(e) {
	if (!switchPage || !switchPage.switched) for (var t = e.changedTouches, r = function(r) {
			var e = pointers.find(function(e) {
				return e.id == t[r].identifier
			});
			if (null == e) return "continue";
